...
static bool	is_mine(struct s_grid *grid, coord *pos)
{
  bool		is_a_mine;
...
static int	count_mines(struct s_grid *grid, coord *pos)
{
  coord		mooves[8];
  coord		new_point;
  int		i;
...
static void	search_around(struct s_grid *grid, coord *point)
{
  coord		mooves[8];
  coord		new_point;
  int		i;
...
static bool	search_victory(struct s_grid *grid, coord *point)
{
  coord		pos;
...
bool	select_nothing(struct s_grid *grid, coord *point)
{
  coord	moovement = coordonne(0, grid->width);
  int	left = LEFT;
  int	mines;
...
bool	select_mine(struct s_grid *grid, coord *point)
{
  if (grid->cases[point->y][point->x].display == '?')
    {
      grid->cases[point->y][point->x].display = '!';
